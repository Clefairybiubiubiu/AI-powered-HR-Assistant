#!/usr/bin/env python3
"""
Simple Resume-JD Matcher using pyresparser for structured extraction.
...
class SimplePyresparserProcessor:
    """Simple document processor using pyresparser for structured extraction."""
...
class SimpleTFIDFMatcher:
    """Simple TF-IDF matcher using pyresparser structured data."""
    
    def __init__(self, processor: SimplePyresparserProcessor):
...
class SimpleSemanticMatcher:
    """Simple semantic matcher using pyresparser structured data."""
...
def main():
    ...
    st.set_page_config(
        page_title="Simple Pyresparser Resume-JD Matcher",
        page_icon="🎯",
        layout="wide"
    )
    
    st.title("🎯 Simple Pyresparser Resume-JD Matcher")
    ...
    if st.sidebar.button("🔄 Load Documents") or 'processor' not in st.session_state:
        ...
        with st.spinner("Loading documents with pyresparser..."):
            processor = SimplePyresparserProcessor(data_dir)
            ...
            if "Semantic" in matching_mode:
                if not SENTENCE_TRANSFORMERS_AVAILABLE:
                    ...
                    return
                matcher = SimpleSemanticMatcher(processor)
            else:
                matcher = SimpleTFIDFMatcher(processor)
